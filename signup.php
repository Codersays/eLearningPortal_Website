...
function is_valid_contactno($contactno) {
    // Contact number should contain only digits and have 10 digits
    return preg_match('/^\d{10}$/', $contactno);
}

// Function to validate password
function is_valid_password($password) {
    // Password should contain at least one uppercase letter, one lowercase letter, one digit, and one special character, and be at least 8 characters long
    return preg_match('/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*()_+}{\';":?><,.\]\[-])(?!.*\s).{6,}$/', $password);
}
...
if(isset($_POST['submit']))
{
    ...
    else if (!is_valid_contactno($mobno)) {
        $msg="Invalid contact number";
    }
    // Validate password
    else if (!is_valid_password($_POST['password'])) {
        $msg="Invalid password. Password should contain at least one uppercase letter, one lowercase letter, one digit, and one special character, and be at least 6 characters long.";
    }
    else {
...
<html lang="en">
<head>
<title>Elearning Portal-Signup Page</title>
